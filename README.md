...
Free, open-source, ground-up editor and generator for Apple Wallet `.pkpass` files. What you see in the live preview matches the signed pass Apple Wallet opens — pixel-for-pixel, field-for-field.

Design boarding passes, event tickets, coupons, store cards, and generic passes in the browser, then download a signed `.pkpass` that installs directly on iPhone. No account, no subscription, MIT licensed.

**Live app:** <https://apple-wallet-passes-editor-generator.vercel.app/>

Designed to be forked and embedded in other apps: the spec, layout engine, preview, and generator are separate importable modules with a stable public API.
...
npm test             # full test suite across spec, layout, preview, generator
npm run build        # Next.js production build
```

The editor is the landing page — opening the site drops you straight into it, no splash screen, no click-through.
...
`app/robots.ts` explicitly allows the major AI crawlers (GPTBot, Claude-Web/ClaudeBot, PerplexityBot, Google-Extended, Applebot-Extended, CCBot, Bytespider, …) so the editor shows up in ChatGPT, Claude, Perplexity, and Google's AI Overviews as an answer source. A structured-data `WebApplication` block is injected into `<head>` via `app/layout.tsx`, and `public/llms.txt` gives LLMs a concise project summary. A `/sitemap.xml` is generated by `app/sitemap.ts`.

Anonymous traffic analytics are collected via `@vercel/analytics` — no personal data, no cookies required.
